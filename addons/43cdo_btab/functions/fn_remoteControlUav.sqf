/*
	Name: 43cdo_btab_fnc_remoteControlUav
	...
	Example:
		[] call 43cdo_btab_fnc_remoteControlUav;
*/
...
if (UAVControl cTabActUav select 1 != "GUNNER") then {
	...
	if (!isNull _uavGunner) then {
		...
		// show notification
		["UAV","No gunner optics available",5] call 43cdo_btab_fnc_addNotification;
	};
} else {
	// show notification
	["UAV","Another user has control",5] call 43cdo_btab_fnc_addNotification;
};
