/*
 	Name: 43cdo_btab_fnc_createHelmetCam
	...
	Parameters:
 	...
 	Example:
		["rendertarget12",str player] spawn 43cdo_btab_fnc_createHelmetCam;
*/
...
if (IsNull _newHost) exitWith {
	["cTab_Tablet_dlg",[["mode","HCAM"]]] call 43cdo_btab_fnc_setSettings;
	false
};
...
if (!isNil "cTabHcams") then {
	...
	if (_oldHost isEqualTo _newHost) then {
		...
		_nop = [] call 43cdo_btab_fnc_deleteHelmetCam;
		waitUntil {_nop};
	};
};
