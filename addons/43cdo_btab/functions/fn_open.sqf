/*
	...
	Example:
		...
		[0,"cTab_TAD_dsp",player,vehicle player] call 43cdo_btab_fnc_open;
*/
...
cTabIfOpen = [_interfaceType,_displayName,_player,
	_player addEventHandler ["killed",{[] call 43cdo_btab_fnc_close}],
	_vehicle,nil,nil,nil,nil];

if (_vehicle != _player && (_isDialog || _displayName in ["cTab_TAD_dsp"])) then {
	cTabIfOpen set [5,
		_vehicle addEventHandler ["GetOut",{if (_this select 2 == cTab_player) then {[] call 43cdo_btab_fnc_close}}]
	];
};
...
if (_displayName in ["cTab_TAD_dsp","cTab_TAD_dlg"]) then {
	cTabIfOpen set [6,
		addMissionEventHandler ["Draw3D",{
			...
			// update time
			(_display displayCtrl IDC_CTAB_OSD_TIME) ctrlSetText call 43cdo_btab_fnc_currentTime;
	...
	cTabIfOpen set [6,
		addMissionEventHandler ["Draw3D",{
			...
			(_display displayCtrl IDC_CTAB_OSD_DIR_OCTANT) ctrlSetText format ["%1",[_heading] call 43cdo_btab_fnc_degreeToOctant];
		}]
	];
};
...
if (isClass (configfile >> "CfgPatches" >> "ace_common")) then {
	cTabIfOpen set [8,
		["playerInventoryChanged",{
			_this call 43cdo_btab_fnc_onPlayerInventoryChanged;
		}] call ace_common_fnc_addEventHandler
	];
};
