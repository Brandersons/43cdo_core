/*
	Name: 43cdo_btab_fnc_deleteUserMarker
	...
	Example:
		...
		["bluefor",5] call 43cdo_btab_fnc_deleteUserMarker;
		
		// Client receiving request for marker deletion (from server)
		["bluefor",5,158] call 43cdo_btab_fnc_deleteUserMarker;
*/
...
call {
	...
	if (isServer) exitWith {
		if (isNil "_transactionId") then {
			...
			_userMarkerList = [cTab_userMarkerLists,_encryptionKey] call 43cdo_btab_fnc_getFromPairs;
			...
			if (_removeIndex != -1) then {
				...
				[cTab_userMarkerLists,_encryptionKey,_userMarkerList] call 43cdo_btab_fnc_setToPairs;
				
				// Send deleteUserMarker command to all clients
				cTab_userMarkerTransactionId = cTab_userMarkerTransactionId + 1;
				[[_encryptionKey,_markerIndex,cTab_userMarkerTransactionId],"43cdo_btab_fnc_deleteUserMarker",true,false,true] call bis_fnc_MP;
	...
	if (hasInterface) then {
		[_this,"43cdo_btab_fnc_deleteUserMarker",false,false,true] call bis_fnc_MP;
	};
};
