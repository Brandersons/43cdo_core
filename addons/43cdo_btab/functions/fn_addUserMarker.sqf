/*
	Name: 43cdo_btab_fnc_addUserMarker
	...
	Example:
		...
		["bluefor",[[1714.35,5716.82],0,0,0,"12:00",player]]call 43cdo_btab_fnc_addUserMarker;
		
		// Client receiving marker addition (from server)
		["bluefor",[21,[[1714.35,5716.82],0,0,0,"12:00",player]],157]call 43cdo_btab_fnc_addUserMarker;
*/
...
call {
	...
	if (isServer) exitWith {
		if (isNil "_transactionId") then {
			...
			[cTab_userMarkerLists,_encryptionKey,[[_transactionId,_markerData]]] call 43cdo_btab_fnc_addToPairs;
			
			// Send addUserMarker command to all clients
			[[_encryptionKey,[_transactionId,_markerData],_transactionId],"43cdo_btab_fnc_addUserMarker",true,false,true] call bis_fnc_MP;
			
			// If this was run on a client-server (i.e. in single player or locally hosted), update the marker list
			if (hasInterface && {_encryptionKey == call 43cdo_btab_fnc_getPlayerEncryptionKey}) then {
				call 43cdo_btab_fnc_updateUserMarkerList;
				if ((_markerData select 5) != cTab_player) then {
					["BFT",format ["New marker at #%1",mapGridPosition (_markerData select 0)],20] call 43cdo_btab_fnc_addNotification;
				} else {
					["BFT","Marker added succesfully",3] call 43cdo_btab_fnc_addNotification;
				};
			};
		};
	};
	...
	if (hasInterface && !isNil "_transactionId") exitWith {
		call {
			...
			[cTab_userMarkerLists,_encryptionKey,[_markerData]] call 43cdo_btab_fnc_addToPairs;
			// only update the user marker list if the marker was added to the player's side
			if (_encryptionKey == call 43cdo_btab_fnc_getPlayerEncryptionKey) then {
				call 43cdo_btab_fnc_updateUserMarkerList;
				
				// add notification if marker was issued by someone else
				if ((_markerData select 1 select 5) != cTab_player) then {
					["BFT",format ["New marker at #%1",mapGridPosition (_markerData select 1 select 0)],20] call 43cdo_btab_fnc_addNotification;
				} else {
					["BFT","Marker added succesfully",3] call 43cdo_btab_fnc_addNotification;
				};
			};
	...
	if (hasInterface) then {
		[_this,"43cdo_btab_fnc_addUserMarker",false,false,true] call bis_fnc_MP;
	};
};
