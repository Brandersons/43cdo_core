/*
 	Name: 43cdo_btab_fnc_setSettings
	...
	Parameters:
 	...
 	Example:
		["cTab_Tablet_dlg",[["mapType","SAT"],["mapScaleDsp","4"]]] call 43cdo_btab_fnc_setSettings;
		
		// Update mapWorldPos and update the interface even if the value has not changed
		["cTab_Tablet_dlg",[["mapWorldPos",getPosASL vehicle player]],true,true] call 43cdo_btab_fnc_setSettings;
		
		// Update mapWorldPos and mapScale, but do not update the interface
		["cTab_Tablet_dlg",[["mapWorldPos",getPosASL vehicle player],["mapScaleDsp","2"]],false] call 43cdo_btab_fnc_setSettings;
*/
...
_propertyGroupName = [cTabDisplayPropertyGroups,_this select 0] call 43cdo_btab_fnc_getFromPairs;
...
_commonProperties = [cTabSettings,"COMMON"] call 43cdo_btab_fnc_getFromPairs;
_groupProperties = [cTabSettings,_propertyGroupName] call 43cdo_btab_fnc_getFromPairs;
...
_combinedPropertiesUpdate = [];
{
	...
	_value = _x select 1;
	call {
		_currentValue = [_groupProperties,_key] call 43cdo_btab_fnc_getFromPairs;
		...
		_currentValue = [_commonProperties,_key] call 43cdo_btab_fnc_getFromPairs;
...
if (!isNil "cTabIfOpen") then {
	call {
		...
		if ((([cTabDisplayPropertyGroups,cTabIfOpen select 1] call 43cdo_btab_fnc_getFromPairs) == _propertyGroupName) && {count _combinedPropertiesUpdate > 0}) exitWith {
			[_combinedPropertiesUpdate] call 43cdo_btab_fnc_updateInterface;
		};
		if (count _commonPropertiesUpdate > 0) then {
			[_commonPropertiesUpdate] call 43cdo_btab_fnc_updateInterface;
		};
	};
};
