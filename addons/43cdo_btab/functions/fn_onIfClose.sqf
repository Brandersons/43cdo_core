...
if !(isNil "cTabIfOpen") then {
	...
	if (!cTabIfOpenStart) then {
		if ([_displayName] call 43cdo_btab_fnc_isDialog) then {
			...
			[_displayName,[["mapWorldPos",cTabMapWorldPos],["mapScaleDlg",_mapScale],["dlgIfPosition",_backgroundOffset]],false] call 43cdo_btab_fnc_setSettings;
		};
	};
