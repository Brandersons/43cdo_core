/*
	Name: 43cdo_btab_fnc_getUserMarkerList
	...
	Example:
		...
		[player] call 43cdo_btab_fnc_getUserMarkerList;
		
		// Client receiving the list from the server
		[[userMarkerList],157] call 43cdo_btab_fnc_getUserMarkerList;
*/
...
call {
	...
	if (count _this == 0) exitWith {
		if (hasInterface && !isServer) then {
			[[player],"43cdo_btab_fnc_getUserMarkerList",false,false,true] call bis_fnc_MP;
		};
	};
	...
	if (count _this == 1) exitWith {
		if (isServer && !hasInterface && (cTab_userMarkerTransactionId >= 0)) then {
			[[cTab_userMarkerLists,cTab_userMarkerTransactionId],"43cdo_btab_fnc_getUserMarkerList",_this select 0,false,true] call bis_fnc_MP;
		};
	};
	...
	if (hasInterface) then {call 43cdo_btab_fnc_updateUserMarkerList;};
};
