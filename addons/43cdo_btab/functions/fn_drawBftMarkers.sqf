/*
 	Name: 43cdo_btab_fnc_drawBftMarkers
	...
	Parameters:
 	...
 	Example:
		[_ctrlScreen,0] call 43cdo_btab_fnc_drawBftMarkers;
*/
...
// ------------------ SEPARATED SQUAD MEMBERS ------------------
// Draw markers for separated squad members from other groups
// Check if any device has separated squad markers enabled and if we have any separated members
_anyDeviceEnabled = false;
if (!isNil "cTab_Android_separatedSquadMarkersEnabled" && {cTab_Android_separatedSquadMarkersEnabled}) then {_anyDeviceEnabled = true;};
if (!isNil "cTab_MicroDAGR_separatedSquadMarkersEnabled" && {cTab_MicroDAGR_separatedSquadMarkersEnabled}) then {_anyDeviceEnabled = true;};
if (!isNil "cTab_Tablet_separatedSquadMarkersEnabled" && {cTab_Tablet_separatedSquadMarkersEnabled}) then {_anyDeviceEnabled = true;};

if (_anyDeviceEnabled && {!isNil "cTabBFTseparated"} && {count cTabBFTseparated > 0}) then {
	// Check if CQB mode is active (global hotkey state)
	_cqbMode = if (!isNil "cTab_cqbModeActive") then {cTab_cqbModeActive} else {false};
	
	{
		_unit = _x select 0;
		_pos = getPosASL _unit;
		_icon = _x select 1;
		_text = if (_drawText && !_cqbMode) then {_x select 3} else {""}; // Hide text in CQB mode
		
		// Use a distinct color for separated squad members (orange/amber)
		_separatedColor = [1, 0.6, 0, 0.8]; // Orange color to distinguish from regular squad members
		
		// Draw the separated squad member marker
		_ctrlScreen drawIcon [_icon, _separatedColor, _pos, cTabIconManSize, cTabIconManSize, direction _unit, "", 0, cTabTxtSize, "TahomaB", "right"];
		
		// Draw text label if enabled and not in CQB mode
		if (_text != "") then {
			_ctrlScreen drawIcon ["\A3\ui_f\data\map\Markers\System\dummy_ca.paa", _separatedColor, _pos, cTabIconManSize, cTabIconManSize, 0, _text, 0, cTabTxtSize, "TahomaB", "right"];
		};
	} forEach cTabBFTseparated;
};
