/*
 	Name: 43cdo_btab_fnc_updateInterface
	...
	(Optional)
 	...
 	Example:
		[[["mapType","SAT"],["mapScaleDsp","4"]]] call 43cdo_btab_fnc_updateInterface;
*/
...
_isDialog = [_displayName] call 43cdo_btab_fnc_isDialog;

if (count _this == 1) then {
	_settings = _this select 0;
} else {
	// Retrieve all settings for the currently open interface
	_settings = [_displayName] call 43cdo_btab_fnc_getSettings;
	_interfaceInit = true;
};

_mode = [_settings,"mode"] call 43cdo_btab_fnc_getFromPairs;
if (isNil "_mode") then {
	...
	call {
		...
		if (_x select 0 == "dspIfPosition") exitWith {
			...
			if !(_isDialog) then {
				...
				_backgroundPosition = [_displayName] call 43cdo_btab_fnc_getBackgroundPosition;
				...
				if !((_backgroundPositionX != _backgroundConfigPositionX) isEqualTo _dspIfPosition) then {
					...
					[_displayName,[_xOffset,0]] call 43cdo_btab_fnc_setInterfacePosition;
				};
			};
		};
		// ------------ DIALOG POSITION ------------
		if (_x select 0 == "dlgIfPosition") exitWith {
			...
			if (_isDialog) then {
				...
				if !(_backgroundOffset isEqualTo [0,0]) then {
					// move by offset
					[_displayName,_backgroundOffset] call 43cdo_btab_fnc_setInterfacePosition;
				};
			};
		};
		// ------------ BRIGHTNESS ------------
		...
		if (_x select 0 == "nightMode") exitWith {
			...
			if (_background != "") then {
				...
				if (!_interfaceInit) then {
					_settings pushBack ["brightness",[_displayName,"brightness"] call 43cdo_btab_fnc_getSettings];
				};
			};
		};
			...
			if !(_displayItems isEqualTo []) then {
				...
				call {
					...
					if (_mode == "BFT") exitWith {
						...
						_mapType = [_displayName,"mapType"] call 43cdo_btab_fnc_getSettings;
						_mapIDC = [_mapTypes,_mapType] call 43cdo_btab_fnc_getFromPairs;
						
						_displayItemsToShow pushBack _mapIDC;
						
						_mapTools = [_displayName,"mapTools"] call 43cdo_btab_fnc_getSettings;
						...
						if (!_interfaceInit) then {
							if (_isDialog) then {
								_settings pushBack ["mapScaleDlg",[_displayName,"mapScaleDlg"] call 43cdo_btab_fnc_getSettings];
								_settings pushBack ["mapWorldPos",[_displayName,"mapWorldPos"] call 43cdo_btab_fnc_getSettings];
							};
						};
					};
					// ---------- _NOT_ BFT -----------
					if (_isDialog) then {
						...
						if (count _mapTypes > 1) then {
							_targetMapName = [_displayName,"mapType"] call 43cdo_btab_fnc_getSettings;
							_targetMapIDC = [_mapTypes,_targetMapName] call 43cdo_btab_fnc_getFromPairs;
							...
							if (ctrlShown _targetMapCtrl) then {
								...
								[_displayName,[["mapWorldPos",cTabMapWorldPos],["mapScaleDlg",_mapScale]],false] call 43cdo_btab_fnc_setSettings;
							};
						};
					};
					// ---------- UAV -----------
					if (_mode == "UAV") exitWith {
						...
						if (!_interfaceInit) then {
							_settings pushBack ["uavCam",[_displayName,"uavCam"] call 43cdo_btab_fnc_getSettings];
						};
					};
					// ---------- HELMET CAM -----------
					if (_mode == "HCAM") exitWith {
						...
						if (!_interfaceInit) then {
							_settings pushBack ["hCam",[_displayName,"hCam"] call 43cdo_btab_fnc_getSettings];
						};
					};
					// ---------- MESSAGING -----------
					...
					if (_mode == "HCAM_FULL") exitWith {
						...
						_data = [_displayName,"hCam"] call 43cdo_btab_fnc_getSettings;
						_btnActCtrl ctrlSetTooltip "Toggle Fullscreen";
						['rendertarget13',_data] spawn 43cdo_btab_fnc_createHelmetCam;
					};
				};
		...
		if (_x select 0 == "mapScaleDsp") exitWith {
			if (_mode == "BFT" && !_isDialog) then {
				...
				if (_mapScaleKm != (_x select 1)) then {
					[_displayName,[["mapScaleDsp",_mapScaleKm]],false] call 43cdo_btab_fnc_setSettings;
				};
			...
			if ((count _mapTypes > 1) && (_mode == "BFT")) then {
				...
				if (!_interfaceInit && _isDialog) then {
					...
					if ((!ctrlShown _targetMapCtrl) && (_targetMapCtrl != _previousMapCtrl)) then {
						...
						if (isNil "_targetMapWorldPos") then {_targetMapWorldPos = [_previousMapCtrl] call 43cdo_btab_fnc_ctrlMapCenter};
					};
				};
		...
		if (_x select 0 == "uavCam") exitWith {
			if (_mode == "UAV") then {
				_data = _x select 1;
				if (_data != "") then {
					[_data,[[0,"rendertarget8"],[1,"rendertarget9"]]] spawn 43cdo_btab_fnc_createUavCam;
				} else {
					[] call 43cdo_btab_fnc_deleteUAVcam;
				};
			};
		};
		// ------------ HCAM ------------
		if (_x select 0 == "hCam") exitWith {
			...
			if (!isNil "_renderTarget") then {
				_data = _x select 1;
				if (_data != "") then {
					[_renderTarget,_data] spawn 43cdo_btab_fnc_createHelmetCam;
				} else {
		...
		if (_x select 0 == "uavListUpdate") exitWith {
			if (_mode == "UAV") then {
				_data = [_displayName,"uavCam"] call 43cdo_btab_fnc_getSettings;
				...
				if (_data != "") then {
					...
					if (lbCurSel _uavListCtrl == -1) then {
						[_displayName,[["uavCam",""]]] call 43cdo_btab_fnc_setSettings;
					};
				};
			};
		};
			...
			if (_mode == "HCAM") then {
				...
				if (_data != "") then {
					...
					if (lbCurSel _hcamListCtrl == -1) then {
						[_displayName,[["hCam",""]]] call 43cdo_btab_fnc_setSettings;
					};
				};
			};
		};
...
if ((!isNil "_targetMapScale") || (!isNil "_targetMapWorldPos")) then {
	...
	if (isNil "_targetMapWorldPos") then {
		_targetMapWorldPos = [_targetMapCtrl] call 43cdo_btab_fnc_ctrlMapCenter;
	};
...
if (_interfaceInit) then {[] call 43cdo_btab_fnc_processNotifications};
